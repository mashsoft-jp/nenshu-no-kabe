...
現在は20〜64歳、配偶者・扶養・追加控除入力、協会けんぽ・厚生年金に加入済みの給与所得モデルです。通常月と年間は対象期間・計算方式が異なり、年間表示は実際の1〜12月の振込総額ではありません。社会保険加入・被扶養者認定の「壁」は未実装です。
...
扶養条件の入力・対象範囲・検証については [扶養条件の仕様](docs/DEPENDENTS.md) を参照してください。


配偶者・その他控除の仕様と確認資料は [docs/DEDUCTIONS.md](docs/DEDUCTIONS.md)。追加計算テストは統合テストに含まれます。追加画面テストは `PLAYWRIGHT_BROWSERS_PATH="$PWD/.venv/browsers" .venv/bin/python test-deductions-ui.py`。

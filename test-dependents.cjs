...
for(const version of [2,3]){
 ...
 const migrated=U.validateDocument(old);eq(migrated.version,6,'upgrade');eq(migrated.dependentLegacy,true,'migration notice');
...
for(const version of [0,7,'5'])bad(()=>U.validateDocument({...doc,version}));

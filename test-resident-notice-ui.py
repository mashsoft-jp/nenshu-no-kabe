...
with sync_playwright() as p:
    ...
    for width in [1440, 1024, 768, 390, 320]:
        ...
        check(june_saved['version']==6 and june_saved['input']['residentCollectionMode']=='june','v5 includes collection choice')
        ...
        # No payroll withholding preserves annual burden and survives annual edits/import.
        page.locator('#advancedConditions').evaluate('(el)=>el.open=true')
        annual_before=page.locator('#simCurrentNet').inner_text()
        page.locator('#monthlyResidentMode').select_option('none')
        expect(page.locator('#monthlyNet')).to_have_text('408,680円')
        expect(page.locator('#simCurrentNet')).to_have_text(annual_before)
        expect(page.locator('#residentSchedule')).to_be_hidden()
        expect(page.locator('#residentJunePay')).to_be_hidden()
        expect(page.locator('#monthlyResidentManualWrap')).to_be_hidden()
        expect(page.locator('#residentNoticeHint')).to_contain_text('月給からは差し引きません')
        page.locator('#simResidentAnnual').fill('250000')
        expect(page.locator('#monthlyResidentMode')).to_have_value('none')
        no_withholding=saved(page)
        check(no_withholding['version']==6,'v6 supports no withholding')
        load(page,initial)
        load(page,no_withholding)
        expect(page.locator('#monthlyResidentMode')).to_have_value('none')
        expect(page.locator('#monthlyNet')).to_have_text('408,680円')
        check(not page.evaluate('document.documentElement.scrollWidth > innerWidth'),'no withholding narrow layout')
        page.close()
    browser.close()

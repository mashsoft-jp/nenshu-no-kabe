...
for (const mode of ['estimate','previous','manual']) {
 ...
 eq(migrated.version,5,'v2 upgrades to v5');eq(migrated.input.monthlyResidentMode,mode,'migration does not silently link');
...
eq(U.validateDocument(require('./example-bonus.json')).version,5,'real v2 example migrates');

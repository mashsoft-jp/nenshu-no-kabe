...
for (const mode of ['estimate','previous','manual']) {
 ...
 eq(migrated.version,7,'v2 upgrades to v6');eq(migrated.input.monthlyResidentMode,mode,'migration does not silently link');
...
eq(U.validateDocument(require('./example-bonus.json')).version,7,'real v2 example migrates');
